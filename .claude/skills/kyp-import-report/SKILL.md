---
name: kyp-import-report
description: Convert a raw DeFi audit report (Audit Framework V1.x output) into the KYP site's content format. Produces TWO artifacts — a public-facing metadata.json (drives the dashboard) and a site-version long-form markdown (drives the /reports/... page). The raw audit is INPUT ONLY and is not copied to the site verbatim. Enforces non-prescriptive, descriptive framing (no investment advice, no position sizing). Use when the user provides or references a raw audit report and wants it onboarded to the site.
---
